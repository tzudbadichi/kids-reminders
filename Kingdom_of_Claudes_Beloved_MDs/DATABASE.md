...
הפונקציה `handle_new_user` (security definer) יוצרת שורת `profiles` אוטומטית בעת הרשמת משתמש חדש. ההתחברות היא באימייל + סיסמה; שם התצוגה ההתחלתי נלקח מהחלק שלפני ה-`@` באימייל (וניתן לשינוי בהגדרות). מופעלת בטריגר `on_auth_user_created` על `auth.users`. דורש ש-"Confirm email" יהיה כבוי ב-Supabase Auth (כניסה מיידית). שחזור סיסמה דרך מייל המובנה של Supabase.
