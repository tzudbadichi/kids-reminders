...
- **הוספת תזכורת (reminders.js -> renderAdd)** — בחירת ילד, תאריך (**ברירת מחדל: מחר**), טקסט ווצאפ אופציונלי, כפתור **"הדבק מהלוח"** (קורא מהקליפבורד ומריץ AI - הזרימה המעשית לווצאפ, שבו טקסט ניתן רק להעתקה), כפתור "חלץ עם AI", ורשימת פריטים נערכת (chips, דרך `chipEditor` משותף). ה-AI ממלא מראש את הפריטים ומעדכן את התאריך אם זוהה בהודעה. שמירה ב-`saveMerged`: מאחדת לשורה אחת לכל (ילד, תאריך) - אם כבר קיימת תזכורת לאותו ילד ויום, הפריטים מתמזגים אליה (ללא כפילויות).
